...
function addLog(action, details) { auditLog.unshift({ time: new Date().toLocaleString(), action, details }); if (auditLog.length > 20) auditLog.pop(); }

function getSidebarFilteredEmployees() { return employees.filter(emp => { if (currentFilters.job && !emp.designation.toLowerCase().includes(currentFilters.job.toLowerCase())) return false; if (currentFilters.dept && !emp.department.toLowerCase().includes(currentFilters.dept.toLowerCase())) return false; if (currentFilters.team && !emp.team.toLowerCase().includes(currentFilters.team.toLowerCase())) return false; return true; }); }

function getTableFilteredEmployees() { return employees.filter(emp => { if (tableFilters.name && !emp.name.toLowerCase().includes(tableFilters.name.toLowerCase())) return false; if (tableFilters.designation && !emp.designation.toLowerCase().includes(tableFilters.designation.toLowerCase())) return false; if (tableFilters.dept && !emp.department.toLowerCase().includes(tableFilters.dept.toLowerCase())) return false; if (tableFilters.team && !emp.team.toLowerCase().includes(tableFilters.team.toLowerCase())) return false; if (tableFilters.minProfile && emp.profile < parseInt(tableFilters.minProfile)) return false; return true; }); }
...
function updateCharts() {
    ...
    profileChart = new Chart(document.getElementById('profileChartCanvas'), { type: 'doughnut', data: { labels: ['Low (<40%)', 'Medium (40-69%)', 'High (≥70%)'], datasets: [{ data: [low, mid, high], backgroundColor: ['#ef4444', '#f59e0b', '#10b981'] }] }, options: { responsive: true, maintainAspectRatio: true, plugins: { legend: { position: 'bottom', labels: { font: { size: 11 } } } } } });
    deptChart = new Chart(document.getElementById('deptChartCanvas'), { type: 'bar', data: { labels: Array.from(deptMap.keys()), datasets: [{ label: 'Employees', data: Array.from(deptMap.values()), backgroundColor: '#3b82f6', borderRadius: 8 }] }, options: { responsive: true, maintainAspectRatio: true, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 } } } } });
}
...
function renderStats() {
    ...
    document.getElementById("statsCards").innerHTML = `<div class="col-md-3"><div class="stat-card text-center"><i class="fas fa-users fa-2x text-primary"></i><h2 class="fw-bold mt-2">${total}</h2><p class="text-muted mb-0">Total Employees</p><small class="text-success"><i class="fas fa-arrow-up"></i> +12%</small></div></div><div class="col-md-3"><div class="stat-card text-center"><i class="fas fa-chart-line fa-2x text-success"></i><h2 class="fw-bold mt-2">${avgProfile}%</h2><p class="text-muted mb-0">Avg Profile</p><small class="text-info"><i class="fas fa-chart-line"></i> Active</small></div></div><div class="col-md-3"><div class="stat-card text-center"><i class="fas fa-trophy fa-2x text-warning"></i><h2 class="fw-bold mt-2">${highPerf}</h2><p class="text-muted mb-0">High Performers</p><small class="text-success"><i class="fas fa-medal"></i> Top talent</small></div></div><div class="col-md-3"><div class="stat-card text-center"><i class="fas fa-building fa-2x text-info"></i><h2 class="fw-bold mt-2">${new Set(employees.map(e => e.department)).size}</h2><p class="text-muted mb-0">Departments</p><small class="text-muted"><i class="fas fa-organization"></i> Active</small></div></div>`;
    updateCharts();
}

function renderTables() {
    let sidebarFiltered = getSidebarFilteredEmployees(); renderStats();
    document.getElementById("dashboardTable").innerHTML = sidebarFiltered.map(emp => `<tr><td class="fw-semibold">${emp.id}</td><td><img src="${emp.avatar}" class="avatar" onerror="this.src='https://randomuser.me/api/portraits/lego/1.jpg'"> ${emp.name}</td><td>${emp.designation}</td><td><span class="badge bg-light border px-3 py-1 rounded-pill">${emp.department}</span></td><td>${emp.team}</td><td>${emp.supervisor}</td><td>${renderProfile(emp.profile)}</td></tr>`).join("");
...
function openEmployeeModal(id = null) { editMode = !!id; document.getElementById("modalTitle").innerHTML = editMode ? "<i class='fas fa-edit me-2'></i>Edit Employee" : "<i class='fas fa-user-plus me-2'></i>Add Employee"; if (id) { let emp = employees.find(e => e.id === id); if (emp) { document.getElementById("empId").value = emp.id; document.getElementById("empName").value = emp.name; document.getElementById("empDesignation").value = emp.designation; document.getElementById("empDepartment").value = emp.department; document.getElementById("empTeam").value = emp.team; document.getElementById("empSupervisor").value = emp.supervisor; document.getElementById("empTenure").value = emp.tenure; document.getElementById("empProfile").value = emp.profile; document.getElementById("empShift").value = emp.shift; document.getElementById("empCity").value = emp.city; document.getElementById("empAvatar").value = emp.avatar; } } else { document.getElementById("empId").value = generateId(); document.getElementById("empName").value = ""; document.getElementById("empDesignation").value = ""; document.getElementById("empDepartment").value = ""; document.getElementById("empTeam").value = ""; document.getElementById("empSupervisor").value = ""; document.getElementById("empTenure").value = ""; document.getElementById("empProfile").value = 50; document.getElementById("empShift").value = "Morning"; document.getElementById("empCity").value = ""; document.getElementById("empAvatar").value = "https://randomuser.me/api/portraits/lego/1.jpg"; } employeeModal.show(); }
function saveEmployee() { let id = document.getElementById("empId").value, name = document.getElementById("empName").value.trim(); if (!name) { alert("Name is required!"); return; } let profile = parseInt(document.getElementById("empProfile").value); if (isNaN(profile) || profile < 0 || profile > 100) { alert("Profile must be 0-100"); return; } let newEmp = { id, name, profile, role: editMode ? (employees.find(e => e.id === id)?.role || "viewer") : "viewer", designation: document.getElementById("empDesignation").value.trim() || "Staff", department: document.getElementById("empDepartment").value.trim() || "General", team: document.getElementById("empTeam").value.trim() || "Core", supervisor: document.getElementById("empSupervisor").value.trim() || "Admin", tenure: document.getElementById("empTenure").value.trim() || "0 Yr", shift: document.getElementById("empShift").value.trim() || "Morning", city: document.getElementById("empCity").value.trim() || "Unknown", avatar: document.getElementById("empAvatar").value.trim() || "https://randomuser.me/api/portraits/lego/1.jpg" }; if (editMode) { let idx = employees.findIndex(e => e.id === id); if (idx !== -1) employees[idx] = newEmp; addLog("UPDATE", `${name}`); } else { if (employees.some(e => e.id === id)) { alert("Duplicate ID!"); return; } employees.push(newEmp); addLog("CREATE", `${name}`); } renderTables(); employeeModal.hide(); showToast(`${editMode ? "Updated" : "Added"} ${name}`); }
function deleteEmployee(id) { let emp = employees.find(e => e.id === id); if (confirm(`Delete ${emp?.name} permanently? This action cannot be undone.`)) { employees = employees.filter(e => e.id !== id); addLog("DELETE", `${emp?.name}`); renderTables(); showToast(`Deleted ${emp?.name}`); } }
...
function showHelp() { alert("📘 NexusHR Help Center\n\n🔹 Quick Tips:\n• Add employees using the 'Add Employee' button\n• Edit/Delete using action icons (✏️ 🗑️)\n• Use filters to find specific employees\n• Export data to CSV for reporting\n• Dark mode toggle in top bar\n• Roles: Admin, Editor, Viewer\n\nFor support: support@nexushr.com"); }
function toggleDarkMode() { document.body.classList.toggle("dark-mode"); let isDark = document.body.classList.contains("dark-mode"); localStorage.setItem("theme", isDark ? "dark" : "light"); let btn = document.getElementById("darkModeBtn"); btn.innerHTML = isDark ? '<i class="fas fa-sun"></i>' : '<i class="fas fa-moon"></i>'; showToast(isDark ? "Dark mode enabled" : "Light mode enabled"); }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("dashboardSearch")?.addEventListener("input", function (e) { document.getElementById("tableSearchName").value = e.target.value; updateTableFilters(); });
