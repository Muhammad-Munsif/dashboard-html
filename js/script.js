<script>
    (function () {
      ...
      function addLog(action, details) { auditLog.unshift({ timestamp: new Date().toLocaleString(), action, details }); if (auditLog.length > 20) auditLog.pop(); console.log("Audit:", action, details); }
      ...
      function openModal(id = null) {
        ...
        if (id) { let emp = employeesData.find(e => e.id === id); if (emp) Object.keys(emp).forEach(k => { let el = document.getElementById(`emp${k.charAt(0).toUpperCase() + k.slice(1)}`); if (el) el.value = emp[k]; }); }
      ...
      function saveEmployee() {
        ...
        renderTables(); employeeModalInstance.hide(); app.showToast(`✅ ${isEditMode ? 'Updated' : 'Added'} ${name}`);
      }
      ...
      function deleteEmployee(id) {
        ...
        if (confirm(`⚠️ Permanently delete ${emp?.name}? This action cannot be undone.`)) {
          ...
          renderTables();
          app.showToast(`🗑️ Deleted ${emp?.name}`);
        }
      }
      ...
      function assignRoleToUser() {
        ...
        renderTables();
        app.showToast(`🔒 ${emp.name} role updated to ${newRole.toUpperCase()}`);
        document.getElementById("assignRoleEmpId").value = "";
      }

      function openPermissionsModalGlobal() { permissionsModalInstance.show(); renderPermissionsList(); }
      ...
      function assignRoleToId(empId, role) { let emp = employeesData.find(e => e.id === empId); if (emp) { emp.role = role; addLog("PERMISSION", `Role changed for ${emp.name} to ${role}`); renderTables(); renderPermissionsList(); app.showToast(`🛡️ ${emp.name} now ${role.toUpperCase()}`); } }
      function showAuditLog() { alert("📋 Audit Log (last 10 actions):\n" + auditLog.slice(0, 10).map(l => `${l.timestamp} - ${l.action}: ${l.details}`).join("\n") || "No actions recorded"); }
      function exportCSV() { let filtered = getFiltered(); let headers = ["ID", "Name", "Designation", "Department", "Team", "Supervisor", "Tenure", "Profile%", "Shift", "City", "Role"]; let rows = filtered.map(e => [e.id, e.name, e.designation, e.department, e.team, e.supervisor, e.tenure, e.profile, e.shift, e.city, e.role]); let csv = [headers, ...rows].map(r => r.join(",")).join("\n"); let blob = new Blob([csv], { type: "text/csv" }); let a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "employees_export.csv"; a.click(); URL.revokeObjectURL(a.href); app.showToast("📁 Export complete"); }

      function applyFilters() { currentFilters = { jobTitle: document.getElementById("filterJobTitle").value.trim(), department: document.getElementById("filterDepartment").value.trim(), team: document.getElementById("filterTeam").value.trim(), city: document.getElementById("filterCity").value.trim() }; renderTables(); app.closeFilterSidebar(); app.showToast("🔍 Filters applied"); }
      function resetFilters() { ["filterJobTitle", "filterDepartment", "filterTeam", "filterCity"].forEach(id => document.getElementById(id).value = ""); currentFilters = { jobTitle: "", department: "", team: "", city: "" }; renderTables(); app.showToast("🔄 Filters reset"); }
      ...
      window.app = {
        ...
        alertFeature(msg) { app.showToast(`✨ ${msg}`); },
        ...
        openPermissionsModal: () => openPermissionsModalGlobal(), openPermissionsForUser: (id) => { openPermissionsModalGlobal(); document.getElementById("assignRoleEmpId").value = id; }, assignRole: () => assignRoleToUser(), assignRoleToId: (id, role) => assignRoleToId(id, role), showAuditLog: () => showAuditLog(), showToast: (msg) => showToast(msg)
      };
