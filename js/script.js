...
function renderProfile(percent) {
  ...
  return `<div class="profile-bar ${cls}"><div class="profile-fill" style="width:${percent}%"></div></div><small>${percent}%</small>`;
}

function getRoleBadge(role) {
  let icon = role === "admin" ? "<i class='fas fa-crown'></i>" : (role === "editor" ? "<i class='fas fa-edit'></i>" : "<i class='fas fa-eye'></i>");
  let badgeClass = role === "admin" ? "role-admin" : (role === "editor" ? "role-editor" : "role-viewer");
...
function renderStats() {
  ...
  document.getElementById("statsCards").innerHTML = `
    ...
    <div class="col-md-4"><div class="stat-card"><i class="fas fa-chart-line fa-2x text-success"></i><h2 class="fw-bold mt-2">${avgProfile}%</h2><p class="text-muted">Avg Profile Completion</p></div></div>
  ...
  document.getElementById("deptChart").innerHTML = Array.from(deptMap.entries()).map(([dept, count]) => `
    <div class="d-flex justify-content-between"><span><i class="fas fa-building"></i> ${dept}</span><span class="badge bg-secondary">${count}</span></div>
...
function renderTables() {
  ...
  document.getElementById("dashboardTable").innerHTML = filtered.map(emp => `
    <tr><td>${emp.id}</td><td><img src="${emp.avatar}" class="avatar" onerror="this.src='https://randomuser.me/api/portraits/lego/1.jpg'"> ${emp.name}</td>
  ...
  document.getElementById("usersTable").innerHTML = filtered.map(emp => `
    <tr><td>${emp.id}</td><td><img src="${emp.avatar}" class="avatar"> ${emp.name}</td>
    <td>${emp.designation}</td><td>${emp.department}</td><td>${emp.team}</td>
    <td>${renderProfile(emp.profile)}</td><td>${getRoleBadge(emp.role)}</td>
    <td class="action-icons"><i class="fas fa-edit text-primary" onclick="editEmployee('${emp.id}')"></i> <i class="fas fa-trash-alt text-danger" onclick="deleteEmployee('${emp.id}')"></i> <i class="fas fa-shield-alt" onclick="openPermissionsForUser('${emp.id}')"></i></td></tr>
  `).join("");
}
...
function openEmployeeModal(id = null) {
  editMode = !!id;
  document.getElementById("modalTitle").innerHTML = editMode ? "<i class='fas fa-edit'></i> Edit Employee" : "<i class='fas fa-user-plus'></i> Add Employee";
  ...
  document.getElementById("permissionsList").innerHTML = employees.map(emp => `
    <div class="d-flex justify-content-between align-items-center border-bottom py-2">
      ...
      <div>${getRoleBadge(emp.role)} 
        ...
        <button class="btn btn-sm btn-outline-primary" onclick="assignRole('${emp.id}', 'admin')">Admin</button>
      </div>
    </div>
  `).join('');
}
