 <script>
  ...
  `).join('');
    ...
    <td class="action-icons"><i class="fas fa-edit text-primary" onclick="window.editEmployee('${emp.id}')" title="Edit"></i> <i class="fas fa-trash-alt text-danger" onclick="window.deleteEmployee('${emp.id}')" title="Delete"></i> <i class="fas fa-shield-alt" onclick="window.openPermissionsForUser('${emp.id}')" title="Permissions"></i></td>
   </tr>`).join("");
    }
    ...
    <div class="d-flex justify-content-between align-items-center border-bottom py-2">
      ...
      <div>${getRoleBadge(emp.role)} 
        <button class="btn btn-sm btn-outline-secondary ms-2" onclick="window.assignRole('${emp.id}', 'editor')">Editor</button>
        <button class="btn btn-sm btn-outline-primary ms-1" onclick="window.assignRole('${emp.id}', 'admin')">Admin</button>
      </div>
    </div>
  `).join('');
    }
