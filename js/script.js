<script>
    ...
    // Get employees filtered by sidebar filters
    ...
    // Get employees filtered by table search inputs
    ...
    function updateCharts() {
      ...
      let deptCounts = Array.from(deptMap.values());

      if (profileChart) profileChart.destroy();
      if (deptChart) deptChart.destroy();

      const ctx1 = document.getElementById('profileChartCanvas').getContext('2d');
      profileChart = new Chart(ctx1, {
        type: 'doughnut',
        data: { labels: ['Low (<40%)', 'Medium (40-69%)', 'High (≥70%)'], datasets: [{ data: [low, mid, high], backgroundColor: ['#ef4444', '#f59e0b', '#10b981'], borderWidth: 0 }] },
        options: { responsive: true, maintainAspectRatio: true, plugins: { legend: { position: 'bottom' } } }
      });

      const ctx2 = document.getElementById('deptChartCanvas').getContext('2d');
      deptChart = new Chart(ctx2, {
        type: 'bar',
        data: { labels: deptLabels, datasets: [{ label: 'Employees', data: deptCounts, backgroundColor: '#3b82f6', borderRadius: 8 }] },
  ...
  `;
    ...
    function renderTables() {
      ...
      // Dashboard table (shows sidebar-filtered data)
      document.getElementById("dashboardTable").innerHTML = sidebarFiltered.map(emp => `
    <tr><td class="fw-semibold">${emp.id}</td>
    ...
    <td>${renderProfile(emp.profile)}</td>
    </tr>`).join("");

      // Users table (shows table-filtered data)
      let tableFiltered = getTableFilteredEmployees();
      document.getElementById("usersTable").innerHTML = tableFiltered.map(emp => `
    <td><td class="fw-semibold">${emp.id}</td>
    <td><img src="${emp.avatar}" class="avatar"> ${emp.name}</td>
    <td>${emp.designation}</td><td>${emp.department}</td><td>${emp.team}</td>
    <td>${renderProfile(emp.profile)}</td><td>${getRoleBadge(emp.role)}</td>
    <td class="action-icons"><i class="fas fa-edit text-primary" onclick="editEmployee('${emp.id}')" title="Edit"></i> <i class="fas fa-trash-alt text-danger" onclick="deleteEmployee('${emp.id}')" title="Delete"></i> <i class="fas fa-shield-alt" onclick="openPermissionsForUser('${emp.id}')" title="Permissions"></i></td>
    <tr>`).join("");
    }
  ...
  `).join('');
    ...
    function updateTableFilters() {
      tableFilters = {
        name: document.getElementById("tableSearchName").value.trim(),
        designation: document.getElementById("tableSearchDesignation").value.trim(),
        dept: document.getElementById("tableSearchDept").value.trim(),
        team: document.getElementById("tableSearchTeam").value.trim(),
        minProfile: document.getElementById("tableSearchMinProfile").value.trim()
      };
      renderTables();
    }

    function clearTableFilters() {
      document.getElementById("tableSearchName").value = "";
      document.getElementById("tableSearchDesignation").value = "";
      document.getElementById("tableSearchDept").value = "";
      document.getElementById("tableSearchTeam").value = "";
      document.getElementById("tableSearchMinProfile").value = "";
      tableFilters = { name: "", designation: "", dept: "", team: "", minProfile: "" };
      renderTables();
      showToast("Table filters cleared");
    }

    function exportToCSV() {
      let filtered = getTableFilteredEmployees();
